...
import com.example.banderas.ml.Edificios;
import com.example.banderas.ml.Facultades;
   ...
   /* Intent intent = new Intent(MediaStore.ACTION_IMAGE_CAPTURE);
    ...
    public void PersonalizedModel(View v) {
        try {
            ...
            String mostProbableLabel = probability.get(0).getLabel();
            double mostProbableScore = probability.get(0).getScore();

            for (int i = 0; i < probability.size(); i++) {
                Category category = probability.get(i);
                if (category.getScore() > mostProbableScore) {
                    mostProbableLabel = category.getLabel();
                    mostProbableScore = category.getScore();
                }
            }

            String res = mostProbableLabel ;
    ...
    private void processImage() {
        ...
        try {
            ...
            txtResults.setText("  EDIFICIO: " + res);
            model.close();
        } catch (IOException e) {
            txtResults.setText("Error al procesar Modelo");
        }
